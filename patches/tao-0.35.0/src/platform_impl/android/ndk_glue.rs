...
pub unsafe fn onActivityCreate(
  ...
  let activity_id: i32 = match env.call_method(&activity, "getId", "()I", &[]) {
    Ok(v) => v.i().unwrap_or(0),
    Err(_) => {
      // Android < 9 (API < 28) does not have Activity.getId()
      // Clear the pending Java exception before trying fallback
